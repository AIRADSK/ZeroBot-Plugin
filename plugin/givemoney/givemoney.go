...
func init() {
	...
	// engine.OnRegex(`给予\s(\d+)\s(\d+)`, zero.OnlyGroup).SetBlock(true).
	// 	Handle(func(ctx *zero.Ctx) {
	// 		nickname = ctx.GetThisGroupMemberInfo(
	...
	// 			ouid  = ctx.Event.UserID
	// 			uidre = math.Str2Int64(ctx.State["regex_matched"].([]string)[1])
	// 		)
	// 		switch {
	// 		case omy > 1000:
	// 			ctx.SendChain(message.At(uidgi), message.Text("一次最多给予1000ATRI币哦~"))
	...
	// 			err1 := wallet.InsertWalletOf(uidre, money)
	// 			if err1 != nil {
	// 				ctx.SendChain(message.Text("接受ATRI币出错啦,返回扣除的ATRI币  [ERROR at gm.go:55]:", err1))
	// 				err := wallet.InsertWalletOf(uidgi, money)
	// 				if err != nil {
